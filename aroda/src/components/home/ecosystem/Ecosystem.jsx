...
const Ecosystem = () => {
  ...
  const ecosystemLogos = [
    ...
    { id: 3, src: images.bigAroda, alt: "Aroda Business Pro Logo", class:"notreal" }, 
        ...
        <div className="ecosystem-grid">
          ...
          <div className="grid-top">
            ...
            <div className="image-card">
              {/* INVISIBLE PLACEHOLDER: Forces the card to stay the correct height */}
              <img 
                src={stats[0].image} 
                className="big-aroda-img placeholder-img" 
                alt="placeholder" 
              />
              
              {/* THE ANIMATED LAYERS: These sit perfectly on top and cross-fade */}
              {stats.map((stat, index) => (
                <img 
                  key={stat.id}
                  src={stat.image} 
                  alt={stat.label} 
                  className={`big-aroda-img ecosystem-img-layer ${activeStat === index ? 'active' : ''}`} 
                />
              ))}
            </div>
